...
- `chartDrawings.ts` — user-drawn trendlines/rays/horizontal lines (`chart_drawings` table), anchored by each point's actual bar date (not a raw pixel/time value) so a magnet-snapped drawing stays pinned to that bar's close across re-renders. Web-only — not exposed to the Telegram agent.

## Chart (`src/components/StockChart.tsx`)

Renders via `lightweight-charts` (TradingView's own library, not Recharts — Recharts has no coordinate-conversion API to build drawing tools on). Candles + one `LineSeries` per selected MA indicator, created imperatively in a `useEffect` (`useRef`-held `IChartApi`/`ISeriesApi`, no official React wrapper exists for this library). Drawing tools (trendline/ray/horizontal + magnet toggle) are a hand-built layer on top: an absolutely-positioned `<svg>` overlay renders each persisted drawing as a line, with pixel positions computed every render from `chart.timeScale().timeToCoordinate()`/`series.priceToCoordinate()` and recomputed on pan/zoom (`subscribeVisibleLogicalRangeChange`) and container resize (`ResizeObserver`). Magnet mode resolves a click to the nearest bar via `coordinateToLogical()` and uses that bar's actual date/close instead of the raw hover value. No charting library ships trendline/magnet tools for free — this logic is fully custom regardless of library choice.
